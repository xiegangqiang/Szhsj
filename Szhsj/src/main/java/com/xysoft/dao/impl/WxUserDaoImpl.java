...
public class WxUserDaoImpl extends BaseDaoImpl<WxUser> implements WxUserDao{
	...
	public Pager<WxUser> getWxUsers(PageParam page, String nickname, String groupid, String[] subscribe_time, String sex, String wxid) {
		...
		if(subscribe_time.length != 0 && !subscribe_time[0].equals("") && !subscribe_time[1].equals("")) {
			...
			if(!"".equals(sex)) {
				hql += " and sex=?";
				return this.getForPager(hql, page, wxid, "%" + nickname + "%", "%" + groupid + "%", subscribe_time[0], subscribe_time[1], sex);
			}else return this.getForPager(hql, page, wxid, "%" + nickname + "%", "%" + groupid + "%", subscribe_time[0], subscribe_time[1]);
		}
		if(!"".equals(sex)) {
			hql += " and sex=? order by createDate desc";
			return this.getForPager(hql, page, wxid, "%" + nickname + "%", "%" + groupid + "%", sex);
		}else {
			hql += " order by createDate desc";
			return this.getForPager(hql, page, wxid, "%" + nickname + "%", "%" + groupid + "%");
		}
	}

	public List<WxUser> getWxUserByWx(String wx) {
		return this.find("from WxUser where wx = ?", wx);
	}
