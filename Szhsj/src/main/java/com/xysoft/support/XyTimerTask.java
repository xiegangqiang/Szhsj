...
public class XyTimerTask {
	...
	public void clientInfoTimer() {
		List<Wx> wxs = (List<Wx>) this.getCurrentSession().createQuery("from Wx").list();
		for (Wx wx : wxs) {
			
			if( NullUtils.isNotEmpty(wx.getAppId()) && NullUtils.isNotEmpty(wx.getAppSecret()) ){
				AccessToken accessToken = WeixinCacheUtil.getAccessToken(wx.getId(), wx.getAppId(), wx.getAppSecret());
				if( accessToken != null) {
					List<WxUserGroup> wxUserGroups = this.getCurrentSession().createQuery("from WxUserGroup where wx=?").setParameter(0, wx.getId()).list();
					HashMap<String, WxUserGroup> map = new HashMap<String, WxUserGroup>();
					for (WxUserGroup wxusergroup : wxUserGroups) {
						map.put(wxusergroup.getGroupId(), wxusergroup);
					}
					List<WxUserGroup> groups = WeixinUtil.getWxUserGroups(accessToken.getToken());
					for (WxUserGroup group : groups) {
						WxUserGroup wxg = null;
						if(map.containsKey(group.getGroupId())) {
							wxg = map.get(group.getGroupId());
						}else {
							wxg = new WxUserGroup();
						}
						wxg.setWx(wx.getId());
						BeanUtils.copyProperties(group, wxg, new String[] {"id", "createDate", "wx"});
						this.getCurrentSession().saveOrUpdate(wxg);
					}
				}
			}
