...
public class WxUserServiceImpl implements WxUserService{
	...
	public String getWxUsers(PageParam page, String nickname, String groupid, String[] subscribe_time, String sex) {
		Pager<WxUser> pager = this.wxUserDao.getWxUsers(page, nickname, groupid, subscribe_time, sex, RequestUtil.getWeiXinId());
	...
	public String synchronizeWx() {
		int count = 0;
		Wx wx = this.wxDao.getWx(RequestUtil.getWeiXinId());
		if(NullUtils.isEmpty(wx.getAppId()) && NullUtils.isEmpty(wx.getAppSecret())) {
			return JsonUtil.toRes("授权参数设置不正确");
		}else {
			List<WxUser> wxUsers = this.wxUserDao.getWxUserByWx(RequestUtil.getWeiXinId());
			...
			for (String openid : openids) { 
				...
				wxUser.setWx(RequestUtil.getWeiXinId());
	...
	public String updateWxUser(WxUser param) {
		...
		if(wxUser != null){
			Wx wx = this.wxDao.getWx(RequestUtil.getWeiXinId());
			if(NullUtils.isEmpty(wx.getId()) && NullUtils.isEmpty(wx.getAppSecret())){return JsonUtil.toRes("授权参数未设置");};
			AccessToken accessToken = WeixinCacheUtil.getAccessToken(wx.getId(), wx.getAppId(), wx.getAppSecret());
