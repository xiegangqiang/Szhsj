...
public class ImageMesServiceImpl implements ImageMesService {
	...
	public String getImageMess(PageParam page, String name) {
		Pager<ImageMesClassifyV> pager = this.imageMesClassifyVDao.getImageMesClassifyVs(page, name, RequestUtil.getWeiXinId());
	...
	public String saveImageMes(ImageMes param) {
		...
		if (!"".equals(param.getId())) {
			...
			imageMes.setWx(RequestUtil.getWeiXinId());
		}
		BeanUtils.copyProperties(param, imageMes, new String[]{"id", "wx", "viewcount"});
	...
	public String getClassifys() {
		List<Classify> classfiys = this.classifyDao.getClassifys(RequestUtil.getWeiXinId());
