...
public class DiymenServiceImpl implements DiymenService {
	...
	public String getDiymenes() {
		List<Diymen> diymenes = this.diymenDao.getDiymenes(RequestUtil.getWeiXinId());
	...
	public String saveDiymen(Diymen param) {
		...
		diymen.setWx(RequestUtil.getWeiXinId());
	...
	public String getWx() {
		...
		return JsonUtil.toStringFromObject(wx);
	}

	@Transactional
	public String configWx(Wx param) {
		Wx wx = this.wxDao.getWx(RequestUtil.getWeiXinId());
		wx.setAppId(param.getAppId());
		wx.setAppSecret(param.getAppSecret());
